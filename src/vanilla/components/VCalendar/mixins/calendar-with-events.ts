// Styles
import "./calendar-with-events.scss";
...
import { getDayIdentifier, diffMinutes } from "../utils/timestamp";
import { parseEvent, isEventStart, isEventOn, isEventOverlapping, isEventHiddenOn } from "../utils/events";
import {
    CalendarTimestamp,
    CalendarEventParsed,
    CalendarEventVisual,
...
type VEventsToNodes = <D extends CalendarDaySlotScope>(day: D, getter: VEventGetter<D>, mapper: VEventVisualToNode<D>, timed: boolean) => VNode[] | undefined;
...
export default CalendarBase.extend({
    ...
    computed: {
        ...
        eventTimedFunction(): CalendarEventTimedFunction {
            return typeof this.eventTimed === "function" ? this.eventTimed : (event) => !!event[this.eventTimed as string];
        },
        eventCategoryFunction(): CalendarEventCategoryFunction {
            return typeof this.eventCategory === "function" ? this.eventCategory : (event) => event[this.eventCategory as string];
        },
        eventTextColorFunction(): CalendarEventColorFunction {
            return typeof this.eventTextColor === "function" ? this.eventTextColor : () => this.eventTextColor as string;
        },
    ...
    methods: {
        ...
        updateEventVisibility() {
            ...
            for (const date in eventsMap) {
                ...
                if (hidden) {
                    more.style.display = "";
                    more.innerHTML = this.$vanilla.lang.t(this.eventMoreText, hidden);
                } else {
        ...
        isEventForCategory(event: CalendarEventParsed, category: CalendarCategory): boolean {
            return (
                !this.categoryMode ||
                (typeof category === "object" && category.categoryName && category.categoryName === event.category) ||
