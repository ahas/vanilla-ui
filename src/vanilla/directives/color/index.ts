// Utilities
import { classToHex, isCssColor, parseGradient } from "../../utils/color-utils";
import colors from "../../utils/colors";

// Types
import { VanillaThemeVariant } from "../../types/services/theme";
...
function setTextColor(el: HTMLElement, color: string, currentTheme: Partial<VanillaThemeVariant>) {
...
function setBackgroundColor(el: HTMLElement, color: string, currentTheme: Partial<VanillaThemeVariant>) {
    const cssColor = !isCssColor(color) ? classToHex(color, colors, currentTheme) : color;
...
function setBorderColor(el: HTMLElement, color: string, currentTheme: Partial<VanillaThemeVariant>, modifiers?: BorderModifiers) {
    const cssColor = !isCssColor(color) ? classToHex(color, colors, currentTheme) : color;
...
function setGradientColor(el: HTMLElement, gradient: string, currentTheme: Partial<VanillaThemeVariant>) {
...
function updateColor(el: HTMLElement, binding: VNodeDirective, node: VNode) {
    const currentTheme = node.context!.$vanilla.theme.currentTheme;
